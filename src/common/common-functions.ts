...
export class CommonFunctions {
	...
	public static generateToken(payload: any, secretKey?: string, durationTimeIn_ms?: number) {
		try {
			const sKey = secretKey ? secretKey : (process.env.AUTH_SECRET_KEY as string);

			if (durationTimeIn_ms) {
				const secret: jwt.Secret = sKey as string;

				const options: jwt.SignOptions = {
					expiresIn: durationTimeIn_ms,
				};

				return jwt.sign(payload, secret, options);
			}

			// For the no-expiration branch, still force correct type:
			const secret: jwt.Secret = sKey as string;
