...
import { content as mailTemplate } from './moriarty-error.template.html';
...
export class EmailSenderService {
  ...
  private async sendException(subject: string, errorDescription: string): Promise<void> {
    if (!global.moriartyNotificationSended) {
      if (!process.env.DEV_ENVIRONMENT) {
          ...
          await MailService.send({
            ...
            (error: any, result: any) => {
              if (error) {
                ...
                global.moriartyNotificationSended = true;
                return {
                  code: 'success',
                  detail: 'mail sended'
                };
        ...
        CommonFunctions.printConsoleColor('--  DEV ENVIRONMENT MORIARTY ERROR --', PrintColorType.error);
        CommonFunctions.printConsoleColor('error email notification', PrintColorType.warning);
        ...
        CommonFunctions.printConsoleColor('-- EO DEV ENVIRONMENT MORIARTY ERROR --', PrintColorType.error);
        global.moriartyNotificationSended = true;
      }
    }
  }
  ...
  private async createMailTemplate(errorDescription: string): Promise<string> {
    ...
    return mailtemplate
      ...
      .replace('{{publicKey}}', process.env.GEAR_KEY as string)
