...
export class AuthenticationService implements AuthenticationServiceInterface {
    ...
    private async modifyPassword(
        ...
        try {
            ...
            if (currentPassword) {
                if (dbResult.password && dbResult.password !== require('md5')(currentPassword)) {
                    throw new CredentialsErrorHandling('user data not found');
                }
            }
    ...
    private async registerPassword(
        ...
        try {
            ...
            if (!userInformation) {
                throw new NotFoundDataHandling('The email and the token were not found');
            }
    ...
    private async removeUserCredentials(workingObject: UserModelInterface): Promise<ServiceResultInterface> {
        try {
            let dbResult: UserDocumentInterface = await UserSchema.findById(workingObject._id);
 
            if (!dbResult) {
                throw new NotFoundDataHandling(`User not found for _id ${workingObject._id}`);
            }
    ...
    private async updateCredentials(workingObject: UserDocumentInterface): Promise<any> {
        try {
    ...
    private async verifyCredentials(email: string, password: string): Promise<ServiceResultInterface> {
        try {
            ...
            const authToken = await CommonFunctions.generateToken(
                { user: dbResult._id, token: global.gearInfo.token },
    ...
    private async verifyKeepSessionAlive(userId: string): Promise<ServiceResultInterface> {
        try {
            let dbResult: UserDocumentInterface = await UserSchema.findById(userId).select([
                'isActive',
                'isConfirmed',
                'keepSessionAlive',
            ]);
    ...
    private async verifyToken(verificationToken: string): Promise<ServiceResultInterface> {
        try {
